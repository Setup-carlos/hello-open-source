...
杂志社有规定：正式杂志（`main` 分支）不能直接涂改。你必须：

1. 拿一张新草稿纸（切新分支）
2. 在草稿纸上写文章（改代码）
3. 把草稿纸装进信封，写上"请审核"（开 PR）
4. 编辑审核通过，刊登到正式杂志（Merge）
5. 草稿纸用完扔掉（删分支）
...
是的。没有草稿纸，就不知道你要投稿什么内容。

**一个分支能开几个 PR？**

一个。一张草稿纸只能装一封信。想再投稿，换一张新草稿纸。
...
```bash
git commit -m "feat: add base structure"
...
- **Merge**：保留三次提交记录
- **Squash and merge**：合成一个干净的提交

**一句话总结**

> 分支是你改代码的草稿纸，PR 是你把草稿纸交给别人审核并合并的申请。

### 删除分支会删掉 PR 记录吗？

不会。

删除分支只是扔掉草稿纸，PR 记录会永久保留在 GitHub 上。你仍然可以打开 PR 页面，看到：
